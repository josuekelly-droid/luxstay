...
import { MapPin, Bed, Maximize, Eye, Loader2, ArrowLeft } from 'lucide-react';
...
export default function AppartementsPage() {
  ...
  const fetchAnnonces = async () => {
    try {
      const response = await fetch('/api/biens?type=APPARTEMENT&limit=9');
  ...
  return (
    <main className="min-h-screen pt-24 pb-16 bg-luxury-sand-light">
      <div className="container mx-auto px-4">
        ...
        <h1 className="font-display text-4xl font-bold text-luxury-green-dark mb-2">Appartements</h1>
        <p className="text-gray-500 mb-8">Découvrez tous nos appartements disponibles au Bénin</p>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-gray-500 text-lg">Aucun appartement disponible pour le moment.</p>
              ...
              <Link key={annonce.id} href={`/bien/${annonce.id}`} className="bg-white rounded-2xl shadow-card overflow-hidden hover:shadow-luxury transition group">
                ...
                <div className="p-5">
                  ...
                  <div className="text-xl font-bold text-luxury-green mb-3">
                    ...
                    {annonce.transaction === 'LOCATION' && <span className="text-sm font-normal text-gray-500">/mois</span>}
                  </div>
