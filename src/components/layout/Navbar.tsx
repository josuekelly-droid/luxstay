...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-3 sm:px-4 lg:px-6 2xl:px-8 max-w-7xl 2xl:max-w-[1400px]">
        <div className="flex items-center justify-between h-16 sm:h-20">
          
          {/* Logo */}
          <Link href="/" className="flex items-center gap-1 text-xl sm:text-2xl font-bold flex-shrink-0">
          ...
          <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1 2xl:gap-2">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`flex items-center gap-1.5 xl:gap-2 px-2.5 xl:px-3 2xl:px-4 py-2 rounded-xl text-xs xl:text-sm font-medium transition whitespace-nowrap ${
                  pathname === link.href
          ...
          <div className="hidden lg:flex items-center gap-1.5 xl:gap-2 2xl:gap-3 flex-shrink-0">
            ...
            {session ? (
              <Link
                href="/dashboard"
                className={`flex items-center gap-1.5 px-3 xl:px-4 py-2 rounded-xl text-xs xl:text-sm font-semibold transition whitespace-nowrap ${
                  isScrolled
                    ? 'bg-luxury-green text-white hover:bg-luxury-green-light'
                    : 'bg-white/20 text-white hover:bg-white/30 backdrop-blur-sm'
                }`}
              >
                <LayoutDashboard size={16} className="xl:size-[18px]" />
                <span>Dashboard</span>
              </Link>
            ) : (
              <>
                <Link
                  href="/connexion"
                  className={`flex items-center gap-1.5 px-3 xl:px-4 py-2 rounded-xl text-xs xl:text-sm font-medium transition whitespace-nowrap ${
                    isScrolled
                      ? 'text-luxury-green-dark hover:bg-luxury-green/10'
                      : 'text-white/90 hover:text-white hover:bg-white/10'
                  }`}
                >
                  <LogIn size={16} className="xl:size-[18px]" />
                  <span>Connexion</span>
                </Link>
                <Link
                  href="/inscription"
                  className="flex items-center gap-1.5 bg-luxury-gold text-luxury-green-dark px-3 xl:px-5 py-2 rounded-xl text-xs xl:text-sm font-semibold hover:bg-luxury-gold-dark transition shadow-luxury whitespace-nowrap"
                >
                  <UserPlus size={16} className="xl:size-[18px]" />
